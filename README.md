...
量子暗号という手段もありますが、量子計算機が一般の過程で使われるようになるのはさらに数十年先であることを考えると、
電子系先を使って計算できる暗号の必要性もまだなくなったわけでもないと感じます。

GoppaDecorderで設定したパラメータはn=8192,t=256というもので、すべての組み合わせの数は10^493というくらいあるので
