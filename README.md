...
２．暗号化鍵をG'=SG、Pとして、G',S,G,Pは秘密鍵である。

３．G'の前半をＡ、後半をＢとする。Ｂの異なる全ての列に重み1のエラーが１か所づつ、混入されている。 
つまり、乱数ｒをwt(r)=tとすると、c=rG’には必ず重みtのエラーが入っていることになる。

４．暗号化鍵Ｇ’の各行の後半のどの位置にダミーのエラーe'が入っているか、各行に対するエラー位置のリストを作成し、それも秘密鍵として保持する。

暗号化：

１．乱数r。ここでt<wt(r)<k,|r|=kをとる。s=rG’+mとする。
...
１平文mをwt(m)=tとなるようにとり、ランダムエラーベクトルeをwt(m)+wt(e)=tとなるようにとる。

２．暗号文c=mG'+eとする。

３.暗号化鍵Ｇ’のmに対するダミーエラーからできたエラーパターンをe'とすると、cに含まれるエラーはe+e'。
ここで、wt(e+e')=t。
